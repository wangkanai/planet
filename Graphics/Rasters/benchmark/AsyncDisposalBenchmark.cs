...
public class AsyncDisposalBenchmark
{
	...
	public void Setup()
	{
		...
		_webpSmall.WebPMetadata.IccProfile = new byte[1000]; // 1KB

		_jpegSmall = new JpegRaster(800, 600);
		_jpegSmall.JpegMetadata.IccProfile = new byte[1000]; // 1KB

		_tiffSmall = new TiffRaster(800, 600);
		_tiffSmall.TiffMetadata.ImageDescription = "Small test image";

		_pngSmall = new PngRaster(800, 600);
		_pngSmall.PngMetadata.TextChunks.Add("Description", "Small test image");
		...
		for (int i = 0; i < 100; i++)
		{
			_webpLarge.WebPMetadata.AnimationFrames.Add(new WebPAnimationFrame { Data = new byte[10_000] });
		}
		_webpLarge.WebPMetadata.HasAnimation = true;

		_jpegLarge = new JpegRaster(4096, 4096);
		_jpegLarge.JpegMetadata.IccProfile = new byte[1_500_000]; // 1.5MB

		// Add many EXIF tags
		for (int i = 0; i < 10_000; i++)
		{
			_jpegLarge.JpegMetadata.CustomExifTags.Add(i, $"Large EXIF tag value {i} with substantial content");
		}

		_tiffLarge = new TiffRaster(4096, 4096);
		_tiffLarge.TiffMetadata.ImageDescription = new string('A', 500_000); // 500KB string
		_tiffLarge.TiffMetadata.Make = "Test Camera Manufacturer";
		_tiffLarge.TiffMetadata.Model = "Test Camera Model XYZ";

		// Add many custom tags
		for (int i = 0; i < 5_000; i++)
		{
			_tiffLarge.TiffMetadata.CustomTags.Add(i, $"Large custom tag {i}");
		}
		...
		for (int i = 0; i < 5_000; i++)
		{
			_pngLarge.PngMetadata.TextChunks.Add($"key{i}", new string('X', 200)); // 200 chars each
		}

		// Add custom chunks
		for (int i = 0; i < 500; i++)
		{
			_pngLarge.PngMetadata.CustomChunks.Add($"CHNK{i:D4}", new byte[2000]);
		}
	}
	...
	public void SyncDisposal_JPEG_Small()
	{
		...
		jpeg.JpegMetadata.IccProfile = new byte[1000];
		jpeg.Dispose();
	}
	...
	public void SyncDisposal_TIFF_Large()
	{
		...
		tiff.TiffMetadata.ImageDescription = new string('A', 500_000);
		tiff.Dispose();
	}
	...
	{
		return _webpLarge.WebPMetadata.EstimatedMetadataSize;
	}
	...
	{
		return _webpLarge.WebPMetadata.HasLargeMetadata;
	}
	...
	{
		return _tiffLarge.TiffMetadata.EstimatedMetadataSize;
	}
	...
	{
		return _pngLarge.PngMetadata.EstimatedMetadataSize;
	}
